 ...
 **/
- (BOOL)activate:(CIMConnectorManager *)acimConnectorManager
{
    __block BOOL result = YES;
    
    dispatch_block_t block = ^{
        
        if (cimConnectorManager != nil)
        {
            result = NO;
        }
        else
        {
            cimConnectorManager = acimConnectorManager;
            
            [cimConnectorManager addDelegate:self delegateQueue:moduleQueue];
            [cimConnectorManager registerModule:self];
 ...
 * then simply check the cimConnectorManager variable in your delegate methods.
 *
...
- (void)deactivate
{
    dispatch_block_t block = ^{
        ...
        if (cimConnectorManager)
        {
            [self willDeactivate];
            
            [cimConnectorManager removeDelegate:self delegateQueue:moduleQueue];
            [cimConnectorManager unregisterModule:self];
            
            cimConnectorManager = nil;
        }
    };
...
- (CIMConnectorManager *)cimConnectorManager
{
    ...
    {
        __block CIMConnectorManager *result;
        
        dispatch_sync(moduleQueue, ^{
            result = cimConnectorManager;
        });
